...
CREATE
DATABASE if not EXISTS caseStudy_module3;
USE
caseStudy_module3;
...
INSERT INTO `caseStudy_module3`.`BoPhan` (`IDBoPhan`, `TenBoPhan`)
VALUES ('1', 'quản lí'),
       ('2', 'nhân viên'),
       ('3', 'giám sát'),
       ('4', 'kế toán');

INSERT INTO `caseStudy_module3`.`TrinhDo` (`IDTrinhDo`, `TrinhDo`)
VALUES ('1', 'Tiến Sĩ'),
       ('2', 'Thạc Sĩ'),
       ('3', 'Cao Học'),
       ('4 ', 'Đại Học'),
...
INSERT INTO `caseStudy_module3`.`ViTri` (`IDViTri`, `TenViTri`)
VALUES ('1', 'Chủ tịch'),
       ('2', 'Giám Đốc'),
       ('3', 'Trưởng phòng'),
       ('4', 'Quản lí'),
...
INSERT INTO casestudy_module3.kieuthue (`TenKieuThue`, `Gia`)
VALUES ('Ngày', 1000000),
       ('Tháng', 20000000);
INSERT INTO casestudy_module3.loaidichvu(`TenLoaiDichVu`)
VALUES ('Villa'),
       ('House'),
       ('Room');

INSERT INTO `caseStudy_module3`.`LoaiKhach` (`IDLoaiKhach`, `TenKhach`)
VALUES ('1', 'Vip'),
       ('2', 'Siliver'),
       ('3', 'Gold'),
       ('4', 'Member');

INSERT INTO `caseStudy_module3`.`DichVuDiKem` (`IDDichVuDiKem`, `TenDichVuDiKem`, `Gia`, `DonVi`)
VALUES ('1', 'Karaoke', '100000', '1'),
       ('2', 'Car', '200000', '2'),
       ('3', 'Ho boi', '200000', '3');

INSERT INTO `caseStudy_module3`.`KhachHang` (`IDKhachHang`, `IDLoaiKhach`, `HoTen`, `NgaySinh`, `SoCMND`, `SDT`,
                                             `Email`)
VALUES ('1', '1', 'Hue', '2001-03-31', '12321', '090554213', 'hovanhue@gmail.com'),
       ('2', '2', 'Hung', '2000-03-31', '12323121', '0343554213', 'nguyenvanhung@gmail.com'),
       ('3', '4', 'Hue', '2001-03-31', '12321', '090554213', 'hovanhue@gmail.com');

INSERT INTO `caseStudy_module3`.`DichVu` (`IDDichVu`, `TenDichVu`, `DienTich`, `SoTang`, `SoNguuoiToiDa`, `ChiPhiThue`,
                                          `IDKieuThue`, `IDLoaiDichVu`, `trangthai`)
VALUES ('1', 'Villa', '100', '2', '10', '100000', '1', '1', 'Trong'),
       ('2', 'House', '100', '1', '10', '100000', '2', '2', 'Trong');


-- yêu cầu 2
SELECT *
FROM casestudy_module3.nhanvien
WHERE (HoTen LIKE 'h%' OR HoTen LIKE 'k%' OR HoTen LIKE 't%')
  and length(HoTen) < 15;

-- yêu cầu 3
SELECT *
FROM casestudy_module3.khachhang
WHERE DiaChi IN ('Đà Nẵng' or 'Quảng  Trị')
  AND (curdate() - NgaySinh > (18 * 30 * 365) and curdate() - NgaySinh < (50 * 30 * 365));
...
where loaikhach.tenkhach = 'diamond'
...
WHERE not EXISTS(SELECT hopdong.idhopdong
                 from hopdong
                 where (hopdong.ngaylaphopdong BETWEEN "2019-01-01" and "2019-03-31")
...
where exists(select hopdong.idhopdong
             from hopdong
             where (hopdong.ngaylaphopdong) = "2018" and hopdong.iddichvu = dichvu.iddichvu)
  and not exists(select hopdong.idhopdong
                 from hopdong
                 where (hopdong.ngaylaphopdong) = "201" and hopdong.iddichvu = dichvu.iddichvu);
...
select temp.month,
       count(month (hopdong.ngaylaphopdong)) as so_khach_hang_dang_ki,
       sum(hopdong.tongtien)                 as tong_tien
from (
         select 1 as month
         union
         select 2 as month
...
select distinct dichvudikem.tendichvudikem, dichvudikem.gia, dichvudikem.donvi
from hopdong
         inner join hopdongchitiet on hopdong.idhopdong = hopdongchitiet.idhopdong
...
inner join HopDongChiTiet on HopDong.IDHopDong = HopDongChiTiet.IDHopDong
where not exists(select HopDong.IDHopDong where HopDong.NgayLapHopDong between "2019-01-01" and "2019-06-31")
and exists(select HopDong.IDHopDong where HopDong.NgayLapHopDong between "2019-09-01" and "2019-12-31")
